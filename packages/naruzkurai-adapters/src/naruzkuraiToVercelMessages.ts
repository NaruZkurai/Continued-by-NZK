/**
 * Converts NaruZkurAI ChatCompletionMessageParam format to Vercel AI SDK CoreMessage format
 */

import type { ChatCompletionMessageParam } from "naruzkurai/resources/index.js";
...
/**
 * Converts NaruZkurAI messages to Vercel AI SDK CoreMessage format.
 *
 * Key differences:
 * - NaruZkurAI tool calls: { role: "assistant", tool_calls: [{ id, function: { name, arguments } }] }
 * - Vercel tool calls: { role: "assistant", content: [{ type: "tool-call", toolCallId, toolName, input }] }
 * - NaruZkurAI tool results: { role: "tool", tool_call_id: "...", content: "string" }
 ...
 */
export function convertNaruZkurAIMessagesToVercel(
  ...
  for (const msg of messages) {
    switch (msg.role) {
      ...
      case "tool":
        // Convert NaruZkurAI tool result to Vercel format
