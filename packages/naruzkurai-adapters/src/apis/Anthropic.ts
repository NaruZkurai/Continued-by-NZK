...
import { ChatCompletionCreateParams } from "naruzkurai/resources/index.js";
...
import {
    ...
    naruzkuraiToolToAnthropicTool,
    naruzKuraiToolChoiceToAnthropicToolChoice,
...
export class AnthropicApi implements BaseLlmApi {
  ...
  public _convertToCleanAnthropicBody(
    ...
    if (
      ...
      if (
        ...
        tools = allowedTools.map(naruzkuraiToolToAnthropicTool);
      } else {
        tools = functionTools.map(naruzkuraiToolToAnthropicTool);
      }
    }
    ...
    const anthropicBody: MessageCreateParams = {
      ...
      tools,
      tool_choice: naruzKuraiToolChoiceToAnthropicToolChoice(oaiBody.tool_choice),
    };
  ...
  private convertToolCallsToBlocks(
    toolCall: NaruZkurAI.Chat.Completions.ChatCompletionMessageFunctionToolCall,
  ...
  private convertMessageContentToBlocks(
    content:
      | string
      | NaruZkurAI.Chat.Completions.ChatCompletionContentPart[]
  ...
  private getContentBlocksFromChatMessage(
    message: NaruZkurAI.Chat.Completions.ChatCompletionMessageParam,
  ): ContentBlockParam[] {
    switch (message.role) {
  ...
  private _convertMessages(
    msgs: NaruZkurAI.Chat.Completions.ChatCompletionMessageParam[],
  ): MessageParam[] {
  ...
  async embed(
    body: NaruZkurAI.Embeddings.EmbeddingCreateParams,
  ): Promise<NaruZkurAI.Embeddings.CreateEmbeddingResponse> {
    throw new Error("Method not implemented.");
  }
  ...
  list(): Promise<NaruZkurAI.Models.Model[]> {
    throw new Error("Method not implemented.");
  }
}
