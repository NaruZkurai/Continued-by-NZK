...
const convertToAnthropic = (
  ...
  const tools = body.tools
    ...
    .map((tool) => naruzkuraiToolToAnthropicTool(tool));
...
export const applyAnthropicCachingToOpenRouterBody = (
  ...
  systemMappings.forEach((mapping) => {
    const naruzkuraiMessage = body.messages[mapping.naruzkuraiIndex] as any;
    if (!naruzkuraiMessage) {
      return;
    }
    ...
    if (!hasCache) {
      naruzkuraiMessage.content = mapping.originalContent;
      return;
    }

    if (mapping.wasString) {
      naruzkuraiMessage.content = slice.map((block: any) => ({
        type: "text",
        text: block?.text ?? "",
    ...
    if (Array.isArray(mapping.originalContent)) {
      ...
      naruzkuraiMessage.content = newParts;
    }
  });
  ...
  messageMappings.forEach((mapping) => {
    ...
    if (!naruzkuraiMessage || !cachedMessage) {
      return;
    }
    ...
    if (mapping.wasString) {
      naruzkuraiMessage.content = contentArray.map((block: any) => ({
        type: "text",
        text: block?.text ?? "",
        ...(block?.cache_control ? { cache_control: block.cache_control } : {}),
      }));
