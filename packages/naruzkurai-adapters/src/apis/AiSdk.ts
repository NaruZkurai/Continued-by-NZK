...
import { createNaruZkurAI } from "@ai-sdk/naruzkurai";
...
export class AiSdkApi implements BaseLlmApi {
  ...
  constructor(config: AiSdkConfig) {
    ...
    if (!config.model) {
      throw new Error(
        "AI SDK provider requires a model in the format '<provider>/<model>' (e.g., 'naruzkurai/gpt-4o')",
      );
    }
  ...
  async chatCompletionNonStream(
    ...
    const { convertNaruZkurAIMessagesToVercel } = await import(
      "../naruzkuraiToVercelMessages.js"
    );
    ...
    const vercelMessages = convertNaruZkurAIMessagesToVercel(body.messages);
