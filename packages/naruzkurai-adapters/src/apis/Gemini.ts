...
import {
    convertNaruZkurAIToolToGeminiFunction,
    GeminiChatContent,
...
interface GeminiToolCall
  extends NaruZkurAI.Chat.Completions.ChatCompletionMessageFunctionToolCall {
  extra_content?: {
    google?: {
...
interface GeminiToolDelta
  extends NaruZkurAI.Chat.Completions.ChatCompletionChunk.Choice.Delta {
  extra_content?: {
    google?: {
      thought_signature?: string;
    };
...
export class GeminiApi implements BaseLlmApi {
  ...
  private _oaiPartToGeminiPart(
    part:
      ...
      | NaruZkurAI.Chat.Completions.ChatCompletionContentPartRefusal,
  ): GeminiChatContentPart {
    switch (part.type) {
      case "refusal":
        return {
    ...
    if (!isV1API) {
      ...
      if (oaiBody.tools?.length) {
        ...
        oaiBody.tools.forEach((tool) => {
          try {
            functions.push(convertNaruZkurAIToolToGeminiFunction(tool));
          } catch (e) {
            console.warn(
