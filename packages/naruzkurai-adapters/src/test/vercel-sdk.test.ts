...
// NaruZkurAI AI SDK Tests
if (process.env.NARUZKURAI_API_KEY) {
  describe("NaruZkurAI with AI SDK", () => {
    testAiSdkProvider({
      aiSdkProviderId: "naruzkurai",
      model: "gpt-4o-mini",
  ...
  test("NaruZkurAI tests FAIL - missing NARUZKURAI_API_KEY", () => {
    throw new Error(
      "vercel-sdk.test.ts NaruZkurAI suite requires process.env.NARUZKURAI_API_KEY; not set.",
    );
  });
}
...
if (process.env.ANTHROPIC_API_KEY) {
  ...
  test("Anthropic tests FAIL - missing ANTHROPIC_API_KEY", () => {
    throw new Error(
      "vercel-sdk.test.ts Anthropic suite requires process.env.ANTHROPIC_API_KEY; not set.",
    );
  });
}
...
describe("Contract Compatibility Tests", () => {
  describe("NaruZkurAI: Original vs AI SDK", () => {
    if (!process.env.NARUZKURAI_API_KEY) {
      test("NaruZkurAI comparison tests FAIL - missing NARUZKURAI_API_KEY", () => {
        throw new Error(
          "Contract comparison suite requires NARUZKURAI_API_KEY; not set.",
        );
      });
      return;
    }
    ...
    const aiSdkModel = "naruzkurai/gpt-4o-mini";
    ...
    test("streaming output format matches", async () => {
      ...
      try {
        ...
        console.error("Error in NaruZkurAI AI SDK streaming:", error);
        throw error;
      }

      expect(originalChunks.length).toBeGreaterThan(0);
      console.log(
        `NaruZkurAI - Original chunks: ${originalChunks.length}, AI SDK chunks: ${aiSdkChunks.length}`,
      );
  ...
  describe("Anthropic: Original vs AI SDK", () => {
    if (!process.env.ANTHROPIC_API_KEY) {
      test("Anthropic comparison tests FAIL - missing ANTHROPIC_API_KEY", () => {
        throw new Error(
          "Anthropic comparison suite requires ANTHROPIC_API_KEY; not set.",
        );
      });
      return;
    }
