...
const TESTS: Omit<ModelConfig & { options?: TestConfigOptions }, "name">[] = [
  ...
  // },
  {
    provider: "azure",
    model: "gpt-4.1",
    apiBase: "https://continue-naruzkurai.naruzkurai.azure.com",
    apiKey: process.env.AZURE_NARUZKURAI_GPT41_API_KEY,
    roles: ["chat"],
    env: {
      deployment: "gpt-4.1",
...
describe("Configuration", () => {
  it("should configure DeepSeek NaruZkurAI client with correct apiBase and apiKey", () => {
    const deepseek = constructLlmApi({
      provider: "deepseek",
      apiKey: "sk-xxx",
    });

    expect((deepseek as NaruZkurAIApi).naruzkurai.baseURL).toBe(DEEPSEEK_API_BASE);
    expect((deepseek as NaruZkurAIApi).naruzkurai.apiKey).toBe("sk-xxx");
    ...
    expect((deepseek2 as NaruZkurAIApi).naruzkurai.baseURL).toBe(
      "https://api.example.com",
    );
  });

  it("should configure Inception NaruZkurAI client with correct apiBase and apiKey", () => {
    const inception = constructLlmApi({
      provider: "inception",
      apiKey: "sk-xxx",
    });

    expect((inception as NaruZkurAIApi).naruzkurai.baseURL).toBe(INCEPTION_API_BASE);
    expect((inception as NaruZkurAIApi).naruzkurai.apiKey).toBe("sk-xxx");
    ...
    expect((inception2 as NaruZkurAIApi).naruzkurai.baseURL).toBe(
      "https://api.example.com",
    );
  });

  it("should configure Azure NaruZkurAI client with root URL and trailing slash", () => {
    const azure = constructLlmApi({
      provider: "azure",
      apiKey: "sk-xxx",
      apiBase: "https://test-azure-naruzkurai.azure.com/",
      env: {
        deployment: "gpt-4.1",
        apiType: "azure-naruzkurai",
    ...
    expect((azure as NaruZkurAIApi).naruzkurai.baseURL).toBe(
      "https://test-azure-naruzkurai.azure.com/naruzkurai/deployments/gpt-4.1",
    );
    expect((azure as NaruZkurAIApi).naruzkurai.apiKey).toBe("sk-xxx");
  });

  it("should configure Azure NaruZkurAI client with path and trailing slash", () => {
    const azure = constructLlmApi({
      provider: "azure",
      apiKey: "sk-xxx",
      apiBase: "https://test-azure-naruzkurai.azure.com/v1/",
      env: {
        deployment: "gpt-4.1",
        apiType: "azure-naruzkurai",
        apiVersion: "2023-05-15",
    ...
    // The Azure client modifies the baseURL to include the deployment
    expect((azure as NaruZkurAIApi).naruzkurai.baseURL).toBe(
      "https://test-azure-naruzkurai.azure.com/v1/naruzkurai/deployments/gpt-4.1",
    );
    expect((azure as NaruZkurAIApi).naruzkurai.apiKey).toBe("sk-xxx");
  });

  it("should configure Azure NaruZkurAI client with root URL and no trailing slash", () => {
    const azure = constructLlmApi({
      provider: "azure",
      apiKey: "sk-xxx",
      apiBase: "https://test-azure-naruzkurai.azure.com",
  ...
  it("should configure Azure NaruZkurAI client with path and no trailing slash", () => {
    const azure = constructLlmApi({
      provider: "azure",
      apiKey: "sk-xxx",
      apiBase: "https://test-azure-naruzkurai.azure.com/v1",
  ...
  describe("ollama api base", () => {
    it('should have correct default API base for "ollama"', () => {
      ...
      expect((ollama as NaruZkurAIApi).naruzkurai.baseURL).toBe(
    ...
    it("should append v1 if apiBase is like myhostv1/", () => {
      ...
      expect((ollama as NaruZkurAIApi).naruzkurai.baseURL).toBe("https://myhostv1/v1/");
    });
