...
export async function runAdapterTest(testCase: AdapterTestCase) {
  ...
  // For non-NaruZkurAI adapters, mock the fetch package
  ...
  // For NaruZkurAI adapters, we need to replace the fetch on the NaruZkurAI client
  if ((api as any).naruzkurai) {
    (api as any).naruzkurai.fetch = mockFetch;
  }
