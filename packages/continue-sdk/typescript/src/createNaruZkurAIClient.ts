...
/**
 * Interface for NaruZkurAI client options with assistant models
 */
interface NaruZkurAIClientOptions extends Record<string, any> {
  /**
...
/**
 * Create and configure an NaruZkurAI client that uses Continue Hub for authentication
 *
 * @param options - NaruZkurAI client options with assistant models
 * @returns Configured NaruZkurAI client
 */
export function createNaruZkurAIClient({
  models: assistantModels,
  organizationId,
  apiKey,
  baseURL = "https://api.continue.dev/",
}: NaruZkurAIClientOptions): NaruZkurAI {
  return new NaruZkurAI({
    apiKey,
