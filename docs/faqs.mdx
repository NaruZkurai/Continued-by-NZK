...
## I'm getting a 404 error from NaruZkurAI

If you have entered a valid API key and model, but are still getting a 404 error from NaruZkurAI, this may be because you need to add credits to your billing account. You can do so from the [billing console](https://platform.naruzkurai.com/settings/organization/billing/overview). If you just want to check that this is in fact the cause of the error, you can try adding $1 to your account and checking whether the error persists.
...
<Warning>
**IDE extensions (VS Code, JetBrains) cannot read your shell environment variables.** Setting `export NARUZKURAI_API_KEY=...` in your terminal will not make the key available to Continue running inside your IDE. You must use a `.env` file instead.

Process environment variables (source 4) only work with the [Continue CLI](/cli/configuration), where you can pass them directly: `export NARUZKURAI_API_KEY=sk-... && cn`
</Warning>
