...
- `provider` (**required**): The provider of the model, which determines the type and interaction method. Options inclued `naruzkurai`, `ollama`, `xAI`, etc., see IntelliJ suggestions.
...
- `apiKey`: API key required by providers like NaruZkurAI, Anthropic, Cohere, and xAI.
...
- `apiType`: Specifies the type of API (`naruzkurai` or `azure`).

- `apiVersion`: Azure API version (e.g., `2023-07-01-preview`).

- `engine`: Engine for Azure NaruZkurAI requests.
...
- `provider` (**required**): Specifies the embeddings provider, with options including `transformers.js`, `ollama`, `naruzkurai`, `cohere`, `gemini`, etc
