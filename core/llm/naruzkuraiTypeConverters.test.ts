...
} from "naruzkurai/resources/responses/responses.mjs";
import { ChatMessage } from "..";
import {
    isItemType,
    toChatMessage,
    toResponsesInput,
} from "./naruzkuraiTypeConverters";
...
describe("naruzkuraiTypeConverters", () => {
  describe("toResponsesInput", () => {
    describe("tool calls handling - NaruZkurAI Responses API", () => {
  ...
  describe("toChatMessage", () => {
    ...
    it("should flatten assistant text-only array content to a string", () => {
      ...
      const result = toChatMessage(message, options);
    ...
    it("should keep string content (with empty-content guard)", () => {
      const message: ChatMessage = {
        role: "assistant",
        content: "", // empty content → guard
      } as ChatMessage;
    ...
    it("should keep non-text parts stripped (assistant type cannot hold images)", () => {
      // Assistant messages in the NaruZkurAI API cannot contain image parts.
      // Original behavior filters to text parts only — must be preserved.
      const message: ChatMessage = {
        role: "assistant",
        content: [
          { type: "text", text: "Look at this: " },
          { type: "imageUrl", imageUrl: { url: "data:image/png;base64,xxx" } },
        ],
      } as ChatMessage;
      ...
      const msg = result as { role: string; content: unknown };
      // Non-text parts are stripped; remaining text kept as array
      expect(Array.isArray(msg.content)).toBe(true);
      expect(msg.content).toEqual([{ type: "text", text: "Look at this: " }]);
    });
  });
});
