...
class NaruZkurAI extends BaseLLM {
  ...
  protected _convertArgs(
    ...
    // NaruZkurAI o1-preview and o1-mini or o3-mini:
  ...
  protected _getEndpoint(
    ...
    if (this.apiType?.includes("azure")) {
      ...
      const isAzureNaruZkurAI =
        this.apiType === "azure-naruzkurai" || this.apiType === "azure";

      const path = isAzureNaruZkurAI
        ? `naruzkurai/deployments/${this.deployment}/${endpoint}`
        : endpoint;
  ...
  protected modifyChatBody(
    ...
    if (body.tools?.length) {
      if (this.isFireworksAiModel(body.model)) {
        ...
        // https://docs.fireworks.ai/guides/function-calling#naruzkurai-compatibility
        body.parallel_tool_calls = true;
      }
      // To ensure schema adherence: https://platform.naruzkurai.com/docs/guides/function-calling#parallel-function-calling-and-structured-outputs
  ...
  private _getEmbedEndpoint() {
    ...
    if (this.apiType === "azure") {
      return new URL(
        `naruzkurai/deployments/${this.deployment}/embeddings?api-version=${this.apiVersion}`,
        this.apiBase,
      );
    }
