...
export interface NaruZkurAISubclassConfig {
  providerName: string;
...
function getExpectedUrl(
  config: NaruZkurAISubclassConfig,
  endpoint: string,
  model: string = "gpt-4",
) {
  let baseUrl = config.defaultApiBase || "https://api.naruzkurai.com/v1/";
  if (config.providerName === "azure") {
    return `${baseUrl}naruzkurai/deployments/${model}/${endpoint}?api-version=2023-07-01-preview`;
...
export const createNaruZkurAISubclassTests = (
  ProviderClass: typeof NaruZkurAI,
  config: NaruZkurAISubclassConfig,
) => {
  describe(config.providerName, () => {
    ...
    test("streamChat should send a valid request", async () => {
      const provider = new ProviderClass({
        apiKey: "test-api-key",
        model: "gpt-4",
        apiBase: config.defaultApiBase || "https://api.naruzkurai.com/v1/",
      });
      ...
      await runLlmTest({
        ...
        expectedRequest: {
          url:
            ...
            `${config.defaultApiBase || "https://api.naruzkurai.com/v1/"}embeddings`,
          method: "POST",
