import { FimCreateParamsStreaming } from "@continuedev/naruzkurai-adapters/dist/apis/base";
import {
    ChatCompletion,
...
import type {
    EasyInputMessage,
    Response as NaruZkurAIResponse,
...
export function toChatMessage(
  ...
  if (message.role === "assistant") {
    ...
    // for compatibility with NaruZkurAI-"compatible" servers
...
export function fromResponsesChunk(
  event: ResponseStreamEvent | NaruZkurAIResponse,
): ChatMessage | ChatMessage[] | undefined {
  if (typeof (event as any).type === "string") {
    return handleResponsesStreamEvent(event as ResponseStreamEvent);
  }
  return handleResponsesFinal(event as NaruZkurAIResponse);
}
...
/**
 * Fixes sequencing/ID issues that cause NaruZkurAI Responses API 400 errors:
